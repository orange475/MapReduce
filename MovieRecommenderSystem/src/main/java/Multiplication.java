...
public class Multiplication {
    public static class MultiplicationMapper extends Mapper<LongWritable, Text, Text, DoubleWritable> {
            ...
            for (int i = 0; i < fileStatus.length; i++) {
                ...
                while (line != null) {
                    ...
                    if (movieRelationMap.containsKey(movie1)) {
                        movieRelationMap.get(movie1).add(mr);
                    } else {
                        List<MovieRelation> movieRelationList = new ArrayList<>();
                        movieRelationList.add(mr);
                        movieRelationMap.put(movie1, movieRelationList);
                    }
                    line = br.readLine();
                }
                br.close();
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            //now we need to calculate the score from the movieRelationMap
            for (MovieRelation relation : movieRelationMap.get(movie)) {
                double score = relation.getRelation() * rating;
                score = score / denominator.get(relation.getMovie2());
                //score xiaoshudian hou baoliu 2 wei
